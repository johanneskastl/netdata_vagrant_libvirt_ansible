Vagrant.configure("2") do |config|
  ...
  (1..W).each do |i|
    ...
    # name the VMs
    config.vm.define "netdata-node#{i}" do |node|
      ...
      # set the hostname
      node.vm.hostname = "netdata-node#{i}"
  ...
  # name the VMs
  config.vm.define "netdata-server" do |node|
    ...
    # set the hostname
    node.vm.hostname = "netdata-server"
    ...
    node.vm.provision "ansible" do |ansible|
      ...
      ansible.groups = {
        ...
        "netdata_nodes"   => [ "netdata-node1", "netdata-node2" ]
      }
